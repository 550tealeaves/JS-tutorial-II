...
let myIcon = L.icon({
    iconUrl: 'museum.png',
    iconSize: [30, 30], // <-- controls overall size (width, height)
...
// === FILTER MUSEUMS BY BOROUGH - from chatGPT ===

// Create a single Leaflet layer so we can clear & refill it
const museumLayer = L.geoJSON(null, {
  pointToLayer: (feature, latlng) => L.marker(latlng, { icon: myIcon }), //helps filter results to icons of selected borough
  onEachFeature: function (feature, layer) {
